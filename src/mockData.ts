...
export const INITIAL_CAMPUSES: Campus[] = [
  {
    ...
    image: 'https://aduzucaqmyktrgocxrrj.supabase.co/storage/v1/object/public/psis-assets/tk.JPG',
...
export function getPersistedCampuses(): Campus[] {
  ...
  try {
    const list = JSON.parse(stored) as Campus[];
    // Auto-update images if they are still using the old defaults
    const updatedList = list.map(c => {
      const initial = INITIAL_CAMPUSES.find(ic => ic.id === c.id);
      if (initial && (c.image.includes('unsplash.com') || c.image === '/tk.jpg' || !c.image)) {
        c.image = initial.image;
      }
      return c;
    });
    localStorage.setItem(STORAGE_KEYS.CAMPUSES, JSON.stringify(updatedList));
    return updatedList;
  } catch (e) {
    return INITIAL_CAMPUSES;
  }
