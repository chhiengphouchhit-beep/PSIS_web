          ...
          <div className="space-y-4 max-w-2xl">
            ...
            <p className="text-xs md:text-sm text-slate-500 font-sans leading-relaxed">
              {lang === 'en'
                ...
                : 'យើងជៀសវាងការធុញទ្រាន់នឹងសៀវភៅក្រាស់ៗ។ សិស្សានុសិស្សទទួលបានអាខោនផ្ទាល់ខ្លួនសម្រាប់ការប្រកួតប្រជែងគណិតវិទ្យា KooBits, កូដឌីង CodeMonkey និងបណ្ណាល័យឌីជីថល Raz-Kids។'}
            </p>
          </div>

          <div className="bg-slate-900 text-white p-6 rounded-2xl border border-white/5 shadow-xl space-y-2 text-xs font-sans max-w-sm shrink-0 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-16 h-16 bg-brand-gold/10 rounded-full blur-xl"></div>
            <span className="text-[10px] text-brand-gold font-extrabold uppercase tracking-widest block font-sans">
              ★ SYSTEM SYNCHRONIZATION
            </span>
            <p className="text-slate-300 leading-relaxed font-light pl-0.5">
              Pupils track scores, reading milestones, and software modules directly on our unified administrative control database.
            </p>
          </div>
        </div>

        {/* Dashboard Frame (Inspired by Linear / Stripe) */}
        <div className="bg-slate-950 rounded-3xl border border-slate-800 shadow-2xl p-6 md:p-8 space-y-8 relative overflow-hidden">
          {/* Subtle grid decor list */}
          <div className="absolute inset-0 bg-[linear-gradient(to_right,#1f2937_1px,transparent_1px),linear-gradient(to_bottom,#1f2937_1px,transparent_1px)] bg-[size:4rem_4rem] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)] opacity-35 pointer-events-none"></div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-800 pb-5 relative z-10">
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                <h4 className="font-sans font-extrabold text-[11px] text-slate-400 uppercase tracking-widest">
            ...
            <div className="flex flex-wrap gap-1.5 bg-slate-900 border border-slate-800/80 p-1 rounded-xl">
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all duration-150 cursor-pointer outline-none ${
                      isSelected
                        ? 'bg-slate-800 text-brand-gold border border-slate-700/60 shadow-sm'
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
              ...
              {filteredApps.map((app) => {
                ...
                return (
                  <motion.div
                    ...
                    key={app.id}
                    className="bg-slate-900/65 hover:bg-slate-900 border border-slate-800/80 rounded-2xl p-6 shadow-xl transition-all duration-300 space-y-6 flex flex-col justify-between hover:border-slate-700 active:scale-[0.98]"
                  >
                    <div className="space-y-4">
                      {/* Left Top Logo and Pill */}
                      <div className="flex justify-between items-start">
                        <div className="w-11 h-11 rounded-xl bg-[#112B8C]/40 text-slate-100 flex items-center justify-center border border-white/5 shadow-inner">
                          <IconComponent size={20} className="text-brand-gold stroke-[2]" />
                        </div>
                        
                        <span className="bg-slate-800 text-brand-gold font-sans font-bold text-[9px] uppercase px-2.5 py-1 rounded-md border border-slate-700/50 tracking-wider">
                          {app.gradeLevels}
                        </span>
                      </div>
                    ...
                    <div className="border-t border-slate-800/80 pt-4.5 space-y-2 text-[10px] font-sans">
                      <div>
                        <span className="font-extrabold text-slate-500 uppercase tracking-widest block">{lang === 'en' ? 'Syllabus integration' : 'គោលបំណងសំខាន់'}</span>
                        <span className="text-slate-300 font-semibold">{app.purpose}</span>
                      </div>
                    </div>
            ...
            {filteredApps.length === 0 && (
              <div className="col-span-full rounded-2xl border border-slate-800 bg-slate-900/70 p-8 text-center text-sm font-semibold text-slate-300">
