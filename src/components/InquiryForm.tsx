...
import { sendTelegramLeadNotification } from '../services/telegram';
...
export default function InquiryForm({ lang, onLeadAdded }: InquiryFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Save to LocalStorage (fallback/hybrid persistence)
    ...
    if (isSupabaseConfigured) {
      try {
        await insertInquiry({
          ...
          student_age: parseInt(studentAge, 10) || 0,
          phone,
          ...
          notes: notes || 'Enquired online through landing page.',
          status: 'New',
          assigned_admin: chosenAdvisor
        });
      } catch (err) {
        console.error('Failed to sync inquiry to Supabase database:', err);
      }
    }
    ...
    <div id="apply-now" className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 max-w-xl mx-auto">
      ...
      <div className="px-8 py-4 bg-gray-50 border-b border-gray-100 flex items-center justify-between text-[11px] font-sans font-semibold text-gray-500">
        <div className="flex items-center space-x-2">
          ...
          <span className={step === 1 ? 'text-brand-blue font-bold' : ''}>{lang === 'en' ? 'Inquiry Form' : 'បំពេញព័ត៌មាន'}</span>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className={step === 2 ? 'text-brand-blue font-bold' : ''}>{lang === 'en' ? 'Confirmation' : 'ជោគជ័យ'}</span>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 animate-fadeIn">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-3.5 py-2.5 text-sm rounded-xl bg-slate-50 border ${errors.parentName ? 'border-red-500 bg-red-50' : 'border-slate-200 focus:border-[#112B8C]'} focus:outline-none focus:bg-white text-slate-800 transition-all duration-200`}
              />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3.5 py-2.5 text-sm rounded-xl bg-slate-50 border ${errors.studentName ? 'border-red-500 bg-red-50' : 'border-slate-200 focus:border-[#112B8C]'} focus:outline-none focus:bg-white text-slate-800 transition-all duration-200`}
                />
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3.5 py-2.5 text-sm rounded-xl bg-slate-50 border ${errors.studentAge ? 'border-red-500 bg-red-50' : 'border-slate-200 focus:border-[#112B8C]'} focus:outline-none focus:bg-white text-slate-800 transition-all duration-200`}
                />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-1">
                  {lang === 'en' ? 'Mobile Phone *' : 'លេខទូរស័ព្ទ *'}
                </label>
                <input
                  type="text"
                  ...
                  className={`w-full px-3.5 py-2.5 text-sm rounded-xl bg-slate-50 border ${errors.phone ? 'border-red-500 bg-red-50' : 'border-slate-200 focus:border-[#112B8C]'} focus:outline-none focus:bg-white text-slate-800 font-mono transition-all duration-200`}
                />
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3.5 py-2.5 text-sm rounded-xl bg-slate-50 border ${errors.email ? 'border-red-500 bg-red-50' : 'border-slate-200 focus:border-[#112B8C]'} focus:outline-none focus:bg-white text-slate-800 transition-all duration-200`}
                />
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3.5 py-2.5 text-sm rounded-xl bg-slate-50 border border-slate-200 text-slate-800 focus:outline-none focus:border-[#112B8C] focus:bg-white transition-all duration-200"
              >
            ...
            <button
              type="submit"
              className="w-full bg-brand-gold hover:bg-amber-500 text-brand-dark font-sans font-extrabold text-xs uppercase tracking-widest py-3.5 rounded-xl shadow-md hover:shadow-lg transition-all duration-200 flex items-center justify-center space-x-2 mt-6 cursor-pointer transform hover:-translate-y-0.5"
            >
              <Sparkles size={14} />
