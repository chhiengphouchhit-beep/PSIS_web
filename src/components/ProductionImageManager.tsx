...
import { uploadCmsImage, getCmsAssets, deleteCmsAsset, isSupabaseConfigured, AssetType } from '../lib/supabase';

function mapAssetTypeToCategory(type: string): ImageAssetCategory {
  switch (type) {
...
export default function ProductionImageManager({ campuses = [], onAssetsUpdate }: ProductionImageManagerProps) {
  ...
  async function loadAssets() {
    let normalizedAssets = getPersistedImageAssets().map((asset) => ({
      ...asset,
    ...
    if (isSupabaseConfigured) {
      try {
        const cmsAssets = await getCmsAssets();
        ...
        normalizedAssets = mergeAssets(normalizedAssets, supabaseAssets);
      } catch (err) {
    ...
    const merged = mergeAssets(normalizedAssets, sheetAssets);
    const cleaned = cleanMergedAssets(merged);
    savePersistedImageAssets(cleaned);
    setAssets(cleaned);
  }
  ...
  function persist(nextAssets: ImageAsset[], logMessage: string) {
    const cleaned = cleanMergedAssets(nextAssets);
    savePersistedImageAssets(cleaned);
    setAssets(cleaned);
  ...
  async function handleUploadFile(file: File, target: ImageAsset | null) {
    ...
    try {
      const category = target?.category || form.category;
      const title = target?.title || form.title || file.name;
      const campus = target?.campus || form.campus;
      const priority = target?.priority || Number(form.priority) || 1;

      if (isSupabaseConfigured) {
        let assetType: AssetType = 'gallery';
        switch (category) {
          case 'Hero Banner': assetType = 'hero'; break;
          case 'Campus Image': assetType = 'campus'; break;
          case 'Campus Gallery': assetType = 'gallery'; break;
          case 'Partner Logo': assetType = 'partner'; break;
          case 'AYLA Logo':
          case 'Header Logo': assetType = 'logo'; break;
          case 'News': assetType = 'news'; break;
          case 'Student Life': assetType = 'student-life'; break;
        }

        const cmsAsset = await uploadCmsImage({
          file,
          title,
          type: assetType,
          campus: campus || undefined,
          section: String(priority),
        });

        const nextAsset: ImageAsset = {
          id: String(cmsAsset.id),
          title: cmsAsset.title,
          category,
          campus: cmsAsset.campus || '',
          originalUrl: cmsAsset.url,
          url: cmsAsset.url,
          priority,
          status: 'Active',
          createdAt: cmsAsset.created_at || new Date().toISOString(),
          storageProvider: 'external-url',
        };

        const updated = target
          ? assets.map((asset) => (asset.id === target.id ? nextAsset : asset))
          : [nextAsset, ...assets];

        persist(updated, 'Image uploaded to Supabase Storage!');
        setChangeImageAsset(null);
        setForm((current) => ({
          ...current,
          id: nextAsset.id,
      ...
      // Fallback: Google Drive Apps Script
  ...
  async function handleDelete(assetId: string) {
    ...
    if (isSupabaseConfigured && assetId.includes('-')) {
      const targetAsset = assets.find((a) => a.id === assetId);
      if (targetAsset) {
        try {
          let assetType: AssetType = 'gallery';
          switch (targetAsset.category) {
            case 'Hero Banner': assetType = 'hero'; break;
            case 'Campus Image': assetType = 'campus'; break;
            case 'Campus Gallery': assetType = 'gallery'; break;
            case 'Partner Logo': assetType = 'partner'; break;
          ...
          const cmsAssetPayload = {
            ...
            type: assetType,
            url: targetAsset.url,
            path: targetAsset.originalUrl?.split('/psis-assets/')?.[1] || '',
            campus: targetAsset.campus || null,
            section: targetAsset.priority ? String(targetAsset.priority) : null,
            created_at: targetAsset.createdAt,
          };

          await deleteCmsAsset(cmsAssetPayload as any);
        } catch (err) {
          console.error('Failed to delete asset from Supabase:', err);
          setMessage('Failed to delete from Supabase storage.');
          return;
        }
      }
    }
      ...
      <form onSubmit={handleSubmit} className="grid gap-4 rounded-xl border border-slate-800 bg-slate-900/70 p-5 lg:grid-cols-4">
        ...
        <label className="block lg:col-span-2">
          <span className="mb-1 block text-[10px] font-bold uppercase text-slate-400">
            {isSupabaseConfigured ? 'Upload Image to Supabase Storage' : 'Upload Image to Google Drive'}
          </span>
          <input
            type="file"
            accept="image/*"
            onChange={(event) => {
              ...
              if (file) {
                const selectedLocation = websiteLocations.find((loc) => loc.key === selectedLocationKey);
                const locationAsset = selectedLocation ? findLocationAsset(selectedLocation) : null;
                handleUploadFile(file, locationAsset || null);
              }
          ...
          />
          {uploadingId === 'new' && (
            <p className="mt-1 text-[10px] font-bold uppercase text-brand-gold">
              {isSupabaseConfigured ? 'Uploading to Supabase Storage...' : 'Uploading to Google Drive...'}
            </p>
          )}
        </label>
                        ...
                        <label>
                          ...
                          />
                           {uploadingId === asset.id && (
                            <p className="mt-1 text-[9px] font-bold uppercase text-brand-gold">
                              {isSupabaseConfigured ? 'Uploading to Supabase Storage...' : 'Uploading to Google Drive...'}
                            </p>
                          )}
                        </label>
